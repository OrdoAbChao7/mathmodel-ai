...
def compile_fixture(arguments: list[str]) -> None:
    output = next((Path(arg.split("=", 1)[1]) for arg in arguments if arg.startswith("-output-directory=")), None)
    ...
    (output / f"{jobname}.pdf").write_bytes(make_pdf())
...
def solve(kind: str, raw: dict) -> tuple[dict, dict, list[dict], str]:
    if kind == "optimization":
        profit, constraints = raw["profit"], raw["constraints"]
        feasible = [(x, y) for x in range(5) for y in range(5) if all(row["x"] * x + row["y"] * y <= row["limit"] for row in constraints)]
        x, y = max(feasible, key=lambda pair: profit["x"] * pair[0] + profit["y"] * pair[1])
        objective = profit["x"] * x + profit["y"] * y
    ...
    if kind == "forecasting":
        observations, holdout = raw["observations"], raw["holdout"]
        train, test = observations[:-holdout], observations[-holdout:]
        slope = (train[-1]["value"] - train[0]["value"]) / (train[-1]["time"] - train[0]["time"])
        intercept = train[0]["value"] - slope * train[0]["time"]
        predictions = [slope * item["time"] + intercept for item in test]
        mae = sum(abs(prediction - item["value"]) for prediction, item in zip(predictions, test)) / len(test)
        baseline_mae = sum(abs(train[-1]["value"] - item["value"]) for item in test) / len(test)
    ...
    ranges = {name: (min(row[name] for row in alternatives), max(row[name] for row in alternatives)) for name in directions}
    normalized = {row["id"]: {name: ((ranges[name][1] - row[name]) if directions[name] == "min" else (row[name] - ranges[name][0])) / (ranges[name][1] - ranges[name][0]) for name in directions} for row in alternatives}
    scores = {key: sum(weights[name] * value for name, value in values.items()) for key, values in normalized.items()}
    ranking = [key for key, _ in sorted(scores.items(), key=lambda item: (-item[1], item[0]))]
    sensitivity_weights = {"cost": 0.4, "benefit": 0.3, "reliability": 0.3}
    sensitivity = [key for key, _ in sorted(((key, sum(sensitivity_weights[name] * value for name, value in values.items())) for key, values in normalized.items()), key=lambda item: (-item[1], item[0]))]
...
def run() -> None:
    config = json.loads((ROOT / "mathmodel.json").read_text(encoding="utf-8"))
    kind = config["problem_type"]
    raw_path = ROOT / "data" / "raw" / "input.json"
    raw = json.loads(raw_path.read_text(encoding="utf-8"))
    result, model, validations, claim = solve(kind, raw)
    ...
    for role in ("data", "method", "result", "validation"):
        ...
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(f'<svg xmlns="http://www.w3.org/2000/svg" width="320" height="120"><text x="12" y="60">{kind} {role} evidence</text></svg>\n', encoding="utf-8")
        figures.append({"id": f"F-{role.upper()}", "role": role, "file": path.relative_to(ROOT).as_posix(), "source_data": "data/raw/input.json", "script": "analysis/run.py", "label": f"fig:{kind}-{role}", "claim_ids": ["C-MAIN"], "source_sha256": digest(path), "readability": "text label remains readable in monochrome"})
    validation_ids = [item["id"] for item in validations]
    for item in validations:
        item["question_id"] = "Q1"
        item["failure_case"] = "outside the small fixture domain"
    write_json(ROOT / "artifacts" / "data-audit.json", {"status": "SUCCESS", "files": [{"path": "data/raw/input.json", "sha256": digest(raw_path), "units": "fixture units", "missingness": 0, "preprocessing": "none"}]})
    write_json(ROOT / "artifacts" / "problem-map.json", {"questions": [{"id": "Q1", "objective": config["title"], "inputs": ["data/raw/input.json"], "outputs": ["R-MAIN"], "method": model["method"], "validation": validation_ids, "section": "body", "model_ids": [model["id"]], "result_ids": ["R-MAIN"], "validation_ids": validation_ids, "claim_ids": ["C-MAIN"]}]})
    model["question_id"] = "Q1"
    write_json(ROOT / "artifacts" / "model-registry.json", {"models": [model]})
    write_json(ROOT / "artifacts" / "result-registry.json", {"results": [{"id": "R-MAIN", "value": result, "unit": "fixture units", "precision": 6, "source": "analysis/results.json", "source_sha256": digest(result_path), "field": "all", "question_id": "Q1", "model_id": model["id"], "validation_ids": validation_ids}]})
    write_json(ROOT / "artifacts" / "claim-registry.json", {"claims": [{"id": "C-MAIN", "body": claim, "result_ids": ["R-MAIN"], "validation_ids": validation_ids, "scope": "small deterministic fixture", "failure_case": model["limitation"], "section": "body"}]})
...
if __name__ == "__main__":
    if "--compile" in sys.argv:
        compile_fixture(sys.argv[sys.argv.index("--compile") + 1:])
    else:
        run()

...
class LatexMetricsTests(unittest.TestCase):
    ...
    def test_fresh_miktex_installation_in_process_output_is_reported(self):
        write_complete_audit_project(self.root)
        with patch("mmcore.latex.subprocess.run", return_value=subprocess.CompletedProcess(
            ["xelatex"], 1, stdout="", stderr="It seems that this is a fresh TeX installation.\nlog4cxx: IO Exception"
        )):
            result = compile_latex(self.root, self.root / "paper" / "main.tex", "xelatex", "paper")
        self.assertTrue(any(item["rule"] == "LATEX-ENV-001" for item in result["errors"]))
    ...
    def test_formal_audit_persists_phase6_gates(self):
        write_complete_audit_project(self.root)
        config = json.loads((self.root / "mathmodel.json").read_text(encoding="utf-8"))
        config["execution_mode"] = "competition_assisted"
        (self.root / "mathmodel.json").write_text(json.dumps(config), encoding="utf-8")
        ...
        payload = json.loads(output.getvalue())
        self.assertEqual(exit_code, 1, payload)
        self.assertEqual(payload["writer_package"]["status"], "FAIL")
        self.assertEqual(payload["review_registry"]["status"], "FAIL")
        self.assertTrue(any(gate["rule"] == "G7-PAPER-EVIDENCE-READY-001" for gate in payload["page_gates"]))
        self.assertTrue(any(gate["rule"] == "G8-ADVERSARIAL-REVIEW-001" for gate in payload["page_gates"]))

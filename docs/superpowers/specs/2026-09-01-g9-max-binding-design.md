...
This closes the distinction between an audit-time max page gate and the final release authority: a max-mode submission cannot become `RELEASE_STATUS=PASS` from a report that contains G0–G8 but omits the required max-specific robustness, red-team, and ARS evidence.

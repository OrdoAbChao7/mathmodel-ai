...
Formal G4/G5 validation now requires `artifacts/experiment-registry.json`. Each experiment record binds a stable experiment ID and run ID to a question, model, code hashes, input hashes, the current configuration hash, seed, environment, start/end timestamps, metrics, figures, and result artifacts.
